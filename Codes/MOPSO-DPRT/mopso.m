...
benchmarks = {'DTLZ3', 'DTLZ4', 'DTLZ5', 'DTLZ6', 'DTLZ7', ...
...
for b = 1:num_benchmarks
    ...
    for test = 1:TEST
        ...
        nPop = 200; nRep = 200; w = 0.24; wdamp = 0.99;
        c1 = 0.62; c2 = 1.98; nGrid = 7; alpha = 0.1; gamma = 2;
        u_idx = 1; 
        ...
        empty_particle.Normalization = []; 
        empty_particle.Explore = []; empty_particle.Memory = [];
        ...
        for it = 1:MaxIt
            ...
            pop = [pop; rep];
            if numel(pop) > nPop
                Grid_pop = CreateGrid(pop, nGrid, alpha);
                for p_idx = 1:numel(pop)
                    pop(p_idx) = FindGridIndex(pop(p_idx), Grid_pop);
                end
        ...
        writetable(array2table(ResultsData, 'VariableNames', colNames_res, 'RowNames', rowNames_all), ...
                   'MOPSO-DPRT_Results.csv', 'WriteRowNames', true);
                   
        writetable(array2table(IterData, 'VariableNames', colNames_iter, 'RowNames', rowNames_all), ...
                   'MOPSO-DPRT_Iterations.csv', 'WriteRowNames', true);
                   
    end % End of 30 Tests
end % End of Benchmark Loop
